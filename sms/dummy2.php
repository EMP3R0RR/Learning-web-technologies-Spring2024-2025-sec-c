...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error_message) {
        ...
        try {
            ...
            mysqli_stmt_execute($stmt);
            $user_id = mysqli_insert_id($conn); // Get the new user ID
            mysqli_stmt_close($stmt);

            // Insert into students table
            $query = "INSERT INTO students (user_id, first_name, last_name, age, blood_group, department, address) VALUES (?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($conn, $query);
            mysqli_stmt_bind_param($stmt, "ississs", $user_id, $fname, $lname, $age, $bloodGroup, $department, $address);
            mysqli_stmt_execute($stmt);
            mysqli_stmt_close($stmt);
...
<body>
  ...
  <div class="landing-header">
    <header>
      <h1 onclick="window.location.href='landingpage.html'">Student Management System</h1>
      <nav>
        <button onclick="window.location.href='login.php'">LOGIN</button>
        <button onclick="window.location.href='dummy2.html'">SIGNUP</button>
        <button onclick="window.location.href='aboutus.html'">TEAM</button>
        <button onclick="window.location.href='landingpage.html'">BACK</button>
      </nav>
    </header>
  </div>
